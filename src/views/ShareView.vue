...
const DaysUntilBirthday = computed<string>(() => {
  const birthDayStr = localStorage.getItem("birth's day");
  const birthMonthStr = localStorage.getItem("birth's month");
  const birthYearStr = localStorage.getItem("birth's year");

  if (!birthDayStr || !birthMonthStr || !birthYearStr) {
...
const shareProfile = () => {
  ...
  if (savedTelegramId) {
    shareUrl.value = `https://t.me/GdowskiyBot/TgProfileApp?startapp=${telegramId.value}`;
...
onMounted(async () => {
  ...
  if (savedTelegramId) {
    ...
    try {
      const response = await axios.get(`api/users/get_by_id/${telegramId.value}`);
